...
//	$conn -> connect_errno
	if(!empty($conn->error)) {
		$errorMsg = 'MySQL error - '.$conn->error.'<br /><br />';
		//echo 'error'; 
	}
	else {
		//echo 'true';
		...
		$hd = array();
		for ($i=0; $i < $numfields; $i++) { // Table header
		...
		if($result->num_rows > 0) {
			$res_ct = 0;
			while ($row = $result->fetch_array()) { //if there is a result
...
$select = 'SHOW TABLES FROM '.$_POST['dbName'];
...
//echo $tables[0];
	...
	$tableList .= '<option value="'.$tables[0].'" '.$pick.'>'.$tables[0].'</option>';
}
...
<body>
<form name="query" action="'.$_SERVER['PHP_SELF'].'" method="POST">
...
$qContent .= '</select> Table '.$tableList.'
		...
		<td></td><td>
			<input type="submit" name="subQuery" value="Submit Query"> 
...
<div width="100%">'.$res.'</div></form>';

if($errorMsg)
	echo showMessage($errorMsg);

echo '<div class="moduleBlue"><h1>What is this?</h1><div>';
$msg = 'This section is for advanced users only. See the database structure as well as make changes to the data.';
