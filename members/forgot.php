...
if($_POST['reset']) {
    ...
    if(empty($password)) {
        ...
        if($password != $confirm) { //passwords do not match
            ...
            $updU = 'update users set password="'.$password.'" where username="'.$username.'"';
...
if($_POST['forgot']) {
    ...
    <p>You or somebody has requested to reset your password for the '.$val['businessName'].' Members Area. To reset your password please click on the link below: </p>
    ...
    if(empty($username)) {
        ...
        if(empty($email)) {
            ...
            $u = $resU->fetch_array($resU);
            
            if(mysqli_num_rows($resU) == 0) { //username does not exist
