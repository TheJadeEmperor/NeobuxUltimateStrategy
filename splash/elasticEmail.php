...
if($_GET['e']) { //email address passed in url
    $emailField = '<input type=text id="email" name="email" value="'.$_GET['e'].'" class="textField">';
}
else {
    $emailField = '<input type=text id="email" name="email" value="name@email.com" onclick="if(this.value==\'name@email.com\') this.value=\'\';" class="textField">';
}

$timer = '
    <script language="JavaScript">
    TargetDate =  "'.date('m/d/Y', time()+86400).' 12:00 AM"; //tonight midnight
    BackColor = "none";
    ForeColor = "black";
    CountActive = true;
    CountStepper = -1;
...
<div id="form">
    ...
    <form method="POST" onsubmit="NoExitPage=true;" action="?action=eeManage">
